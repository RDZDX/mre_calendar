...
static void draw_calendar(void) {
    ...
    VMCHAR wdays[7][3] = {"Mo","Tu","We","Th","Fr","Sa","Su"}; // Monday first
    ...
    for (c=0; c<7; ++c) {
        vm_ascii_to_ucs2(wdays1[c], sizeof(wdays1[c]), wdays[c]);
        if (c>=5) { // šeštadienis, sekmadienis
           color.vm_color_565 = VM_COLOR_RED;
        } else {
          color.vm_color_565 = VM_COLOR_WHITE;
        }
        vm_graphic_setcolor(&color);
	vm_graphic_textout_to_layer(layer_hdl[0], START_X + c*CELL_W + 5, START_Y - 15, wdays1[c], vm_graphic_get_character_height());
    }
